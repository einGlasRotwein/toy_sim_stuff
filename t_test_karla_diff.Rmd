...
```{r}
samplesizes <- seq(10, 200, 5) #0-200 in steps of 5
nruns <- 1000
...
results_diff1 <- t_test_simulation(samplesizes, nruns, diff = diff[1])
results_diff2 <- t_test_simulation(samplesizes, nruns, diff = diff[2])
results_diff3 <- t_test_simulation(samplesizes, nruns, diff = diff[3])
```


```{r}
results_summary_diff1 <- results_diff1 %>% 
  mutate(sign = pvalue < .05) %>% 
  group_by(samplesizes) %>% 
  summarise(perc_sign = mean(sign), .groups = "drop") %>%
  mutate(effectsize = diff[1])
results_summary_diff2 <- results_diff2 %>% 
  mutate(sign = pvalue < .05) %>% 
  group_by(samplesizes) %>% 
  summarise(perc_sign = mean(sign), .groups = "drop") %>%
  mutate(effectsize = diff[2])
results_summary_diff3 <- results_diff3 %>% 
  mutate(sign = pvalue < .05) %>% 
  group_by(samplesizes) %>% 
  summarise(perc_sign = mean(sign), .groups = "drop")%>%
  mutate(effectsize = diff[3])
```


```{r}
ggplot() +
  geom_point(data=results_summary_diff1, aes(x = samplesizes, y = perc_sign, colour="red", )) +
  geom_point(data=results_summary_diff2, aes(x = samplesizes, y = perc_sign, colour="blue", )) +
  geom_point(data=results_summary_diff3, aes(x = samplesizes, y = perc_sign, colour="black", )) +
  scale_y_continuous(breaks = seq(0, 1, .1), limits = c(0, 1)) +
  scale_x_continuous(breaks = seq(10, 200, 20)) +
  scale_colour_manual(name  ="Effect size",values = c("black", "blue", "red"), breaks=c("black", "blue", "red"), labels=rev(diff)) +
  xlab("samplesize")
